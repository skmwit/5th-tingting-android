...
internal class CircleIndicator : LinearLayout {
    ...
    private var imageDot: Array<ImageView>? = null
    ...
    fun createDotPanel(count: Int, defaultCircle: Int, selectCircle: Int) {
        ...
        var imageDot = ArrayList<ImageView>(count)
    ...
    fun selectDot(position: Int) {
        ...
        for (i in imageDot!!.indices) {
            if (i == position) {
                ...
                if (imageDot!![i].getTag(imageDot!![i].id) as Boolean == true) {
